...
*********************************************************************************
* creating brdr for only CAN-USA
gen brdr=1 if exporter=="USA" && importer=="CAN"
replace brdr=1 if exporter=="CAN" && importer=="USA"
replace brdr=0 if brdr==.

ppml trade LN_DIST CNTG BRDR brdr exp_fe_* imp_fe_1-imp_fe_68, iter(30) noconst
estimates store brdr1

*ppml trade LN_DIST CNTG BRDR brdr exp_fe_* imp_fe_1-imp_fe_68, iter(30) noconst
*estimates store brdr2

*esttab brdr1 brdr2 ///
    using "/home/bepp/hchulkim/Documents/gravity/output/tables/tableQ3_a.tex", ///
    replace label nodepvar nogaps compress booktabs ///
    b(2) se(2) star(* 0.10 ** 0.05 *** 0.01) ///
    keep(LN_DIST CNTG BRDR brdr) ///
    varlabels(LN_DIST "log(distance)" ///
              CNTG "Contiguity" ///
	      BRDR "Border" ///
	      brdr "USA-CAN border") ///
...
******************************************************************************
gen t_ij_BLN=exp(_b[LN_DIST]*LN_DIST+_b[CNTG]*CNTG+_b[BRDR]*BRDR + _b[brdr]*brdr)
...
* Start counterfactual from the baseline BRDR
gen BRDR_CFL = BRDR

* Erase border between CAN and USA
replace BRDR_CFL = 0 if exporter=="USA" & importer=="CAN"
replace BRDR_CFL = 0 if exporter=="CAN" & importer=="USA"

* Construct counterfactual trade costs
gen t_ij_CFL  = exp(_b[LN_DIST]*LN_DIST + _b[CNTG]*CNTG + _b[BRDR]*BRDR_CFL)
...
dataout, save(border_indexes) word replace dec(2)
